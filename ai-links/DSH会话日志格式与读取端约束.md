...
>
> 2026-09-13 复核补：触发条件已在 §九 第 2 条**补全并定论**——判据是 `scanZstdFrames`（`J:1298-1361`）的六个「字节不够」出口对「魔数 / 保留帧头位 / 保留块类型」三个硬拒出口；整字节翻转实测 3 容忍 / 3 硬拒，定向 `size` 越界一律容忍。**「能打开」不等于「内容完整」这句结论不变，且现在有了可判定的边界。**
...
> [!success] 残余复核（2026-09-13）：上表四行逐条定论（第 4 行留一处代际残留）
> 复核方式与 §〇 一致：**合成日志 + 真实编解码器 + 真实后端**，夹具根 `%TEMP%\dsh-kbrecheck\`（脚本 `run.mjs` / `run2.mjs` / `run4.mjs` / `run5.mjs`，逐字输出同目录 `out.txt`）；**未读取任何真实会话内容**。
>
> 1. **seeded 丢 inherited `session/end-seed` 的实际报错文本**：实测落到用户眼前的是
>    `SessionPersistenceCorruptionError: session "<sid>": stored log is corrupt: SessionFormatError: released v2 seeded Session lacks an inherited end-seed marker`。
>    ⚠️ 是 **v2 文案**（`V12:256-260`），不是 `V23:572-577` 的「format v3 seeded Session lacks an accepted inherited end-seed marker」——v3 包装的 `finish()` 只在**流式解码已积累 issue** 时才走自己那句，无 issue 时委托给 v2 解码器。两条引用都要留，但**默认路径给的是 v2 那句**。
> 2. **块头损坏落入「静默截断」的完整触发条件**：条件可完全由 `scanZstdFrames`（`J:1298-1361`）的**六个「字节不够」出口**刻画——走查在帧起始 / 帧描述符 / 帧头余量 / 块头 / 块体 / 校验和任一处读越界，就返回 `tornStart`（⇒ 容忍、静默截断）；**只有**魔数不符、保留帧头位（`descriptor & 24`）、保留块类型（`blockType === 3`）才是硬拒。故「块头损坏」本身不决定结果，取决于损坏把 `size` / `last` 推向哪边。实测：3 帧 × 3 个块头字节各**整字节翻转** ⇒ **3 例容忍（6 条 → 0 条）、3 例硬拒（`frame at byte N failed validation`）**；定向把 `size` 抬到越界 ⇒ **一律容忍**（帧#1 → 0 条、帧#2 → 3 条）。原文「事件数从 8 变成 0/3/6」的观察成立，现在补上了判据而非仅样本。
> 3. **`none`（明文 `.jsonl`）编码的容忍差异**：**行级语义完全同规**——完整行 JSON 损坏 → `unparsable committed event at line N`；未知类型 → `SessionFormatUnsupportedError`；`seq` 不密集 → `seq gap (expected 0, got 3)`；**物理未写完的末行（截断 12 字节）→ 静默略过、`open()` 照常成功**（6 条 → 5 条）。**实质差异只在容器层**：明文没有帧结构，「帧校验和失败 / 保留块类型 / 帧头保留位 / 头帧不是恰好一行」这几条硬拒路径整体不存在，也就没有 `none` 版的第 2 条。附注：`.jsonl` 与 `.jsonl.zstd` **不能同根混放**（报 `use a separate root`），换编码去读旧后缀文件直接拒绝。
> 4. **v0/v1/v2 历史代的读取 / 迁移行为**：迁移是**读取期、内存内**的——`open(id,'read')` **不会**在盘上生成新代文件（实测：目录里只有 `session.v2.jsonl.zstd`，`open()` 成功后目录内容不变）。三条硬规则实测：① 文件名版本必须与 header 一致，否则 `resolved JSONL source filename identifies v2, but its header identifies v3`；② 同目录多代并存取**版本号最大**的 canonical 文件，旧代原样保留；③ 迁移链是**拒绝而非降级**（准入失败时原文案「source v2 artifact remains unchanged」）。另：v2 行经迁移后**事件形态会变**（夹具 6 条 v3 事件 → 读出 7 条，多出一条 `system/message`），改写工具不能假设「盘上代 == 读出代」。
> 残留（具体判据）：只实测了 **v2** 起跑的迁移链；**v0 / v1** 两代未各造合法夹具，逐代差异待补。
...
| 结项 | §九 第 1 行「seeded 丢 inherited `end-seed` 的实际报错文本」未验证，只给了源码推断 | 合成 seeded 夹具实测：报错为 `SessionFormatError: released v2 seeded Session lacks an inherited end-seed marker`（**v2 文案**；`V23:572-577` 的 v3 文案只在流式已积累 issue 时才走）。夹具 `%TEMP%\dsh-kbrecheck\` |
| 结项 | §九 第 2 行「块头损坏落入静默截断的完整触发条件」未验证，只测 3 帧 × 3 字节 | 补全为 `scanZstdFrames`（`J:1298-1361`）六处「字节不够」⇒ 容忍 / 三处「魔数·保留帧头位·保留块类型」⇒ 硬拒 的判据；整字节翻转 3:3，定向 `size` 越界一律容忍 |
| 结项 | §九 第 3 行「`none` 编码下的容忍差异」未验证 | 实测：行级语义与 `zstd` **同规**（完整行损坏/未知类型/seq gap 硬拒，物理未写完末行静默略过）；差异只在容器层——明文无帧结构，故无校验和/保留块类型类硬拒路径。另发现两份后缀不可同根混放 |
| 结项 | §九 第 4 行「v0/v1/v2 历史代迁移行为」本轮未覆盖 | 实测 v2 起跑：迁移为**读取期内存内**、不在盘上生成新代（`session.v2` 目录 `open()` 后内容不变）；文件名版本必须与 header 一致；多代并存取最大版本；拒绝而非降级。残留：v0/v1 未各造夹具 |
| 加厚 | §六 的容忍路径反向风险只到「触发条件依赖位翻转位置」 | 就地补指针：触发条件已由 §九 第 2 条补全为可判定边界，原结论「能打开≠内容完整」不变 |
