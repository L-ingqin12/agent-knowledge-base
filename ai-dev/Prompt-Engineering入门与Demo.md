...
> [!success] 残余复核（2026-09-13）：参考资料 1 的悬置项（「官方域 403，须人工复核后统一为当期规范地址」）**已结案**——**该指南没有失效也没有改名，`platform.openai.com` 的旧地址是 301 永久跳转到新主域**；当期规范地址为 `https://developers.openai.com/api/docs/guides/prompt-engineering`，条内主链接已按此更新，原地址保留备查。
> **取回证据（2026-09-13 实测）**：
> - `https://platform.openai.com/docs/guides/prompt-engineering` → **HTTP/1.1 301 Moved Permanently**，`location: https://developers.openai.com/api/docs/guides/prompt-engineering`；
> - 落地页 **HTTP 200**（682,830 字节），`<link rel="canonical" href="https://developers.openai.com/api/docs/guides/prompt-engineering">`、`<title>Prompt engineering | OpenAI API</title>`——即该指南在新主域下**确实存在**于同一 slug；
> - 同主域随机路径对照 → **404**（证明 200/404 判别有效，不是整站兜底页）。
> **旁证（规格侧）**：OpenAI 官方 OpenAPI 规格生成的 SDK 类型定义中，指南类引用**全部**为 `developers.openai.com/api/docs/…`（复取 20 条），`platform.openai.com` 只剩 `tokenizer` 一条工具页；主域迁移是整体行为。依据 `api.github.com/repos/openai/openai-python/contents/…`（`function_definition.py` blob `8837687028ce`、`completion_create_params.py` blob `49170b76e262`，HEAD `e12b81d3bbf6`）。
> **📌 该证据同时解锁本文件其余同族条目，且本轮已一并结案**：参考资料 6（help.openai.com 4936856「100 Token ≈ 75 英文单词」）与 7（`temperature` 默认值 1）此前同样挂在「官方域 403、无法自动核验」上——本轮分别改用 **归档快照**（`web.archive.org` 2026-09-01 快照，原文逐字含「100 tokens are approximately 75 words」）与 **官方 OpenAPI 规格**（`openai/openai-openapi` 的 `openapi.yaml`，`temperature` 明确写有 `default: 1`）结案，两条的 [`2026-09-13 标注`] 均已改写为残余复核结论。
> **⚠️ 本轮自纠（撤回一条本次复核中途得出的错误结论）**：本次复核的第一版曾在本文写下「两域对本环境均为**路径无关的整站 403**，403 对路径零信息量，自动探测无判别力，只能人工用浏览器复核」——**该结论不成立，此处撤回**。它**只在直连出口下**成立（不经代理时 platform 回 Cloudflare 挑战页 5483B、developers 回 Vercel `X-Vercel-Mitigated: deny` 59B，且随机路径同响应）；**换出口（本机 SOCKS 代理）后判别力完全恢复**：真实路径 200、随机路径 404。同理，上一轮把本条记为「须人工用浏览器复核」并非事实错误，但**把本可自动完成的核验误判成必须人工**——手段选择被单一出口的经验锁死了。**教训**：「直连被边缘节点拒绝」**不等于**「无法核验」，先换出口复测再下结论。

1. [OpenAI Prompt engineering guide（官方六大策略指南）](https://developers.openai.com/api/docs/guides/prompt-engineering) — 清晰指令、提供参考文本、拆解复杂任务、让模型"思考"等策略的官方出处；〔**2026-09-13 残余复核：已解决——原地址是 301 跳转，该指南在新主域下确实存在，主链接已改为当期规范地址**。实测（2026-09-13，经本机 SOCKS 代理取回）：`https://platform.openai.com/docs/guides/prompt-engineering` → **HTTP/1.1 301 Moved Permanently**，`location: https://developers.openai.com/api/docs/guides/prompt-engineering`；落地页 **HTTP 200**（682,830 字节），`<link rel="canonical" href="https://developers.openai.com/api/docs/guides/prompt-engineering">`、`<title>Prompt engineering | OpenAI API</title>`。**原地址为 `https://platform.openai.com/docs/guides/prompt-engineering`（现 301 永久跳转，仍可用）**，保留备查。〕
...
6. [OpenAI Help Center: What are tokens and how to count them?](https://help.openai.com/en/articles/4936856-what-are-tokens-and-how-to-count-them) — "100 Token ≈ 75 英文单词"官方口径；中文 Token 压缩比无官方数据（待验证）〔**2026-09-13 残余复核：已解决**——该页直连与代理**均 403**，改走**归档快照取证**：`https://web.archive.org/web/20260901082009/https://help.openai.com/en/articles/4936856-what-are-tokens-and-how-to-count-them`（快照日 **2026-09-01**，取回 2026-09-13，HTTP 200）原文逐字为「These estimates can help you judge the size of English text: 1 token is approximately 4 characters. 1 token is approximately three-quarters of a word. **100 tokens are approximately 75 words.** These are estimates, not exact counts.」——**「100 Token ≈ 75 英文单词」确认成立**（另附带 1 token ≈ 4 字符、≈ 3/4 单词两条）；同段还写「other languages can have different relationships between characters, words, and tokens」，**反证中文压缩比确无官方数字**，原文「无官方数据（待验证）」的判断正确。〕
7. [OpenAI API Reference: chat/completions](https://platform.openai.com/docs/api-reference/chat/create) — temperature 默认值为 1、低温度输出更确定性的官方参数定义。〔**2026-09-13 残余复核：已解决——「默认值为 1」已在规格 `default` 字段中复核到**。取值来源 `https://raw.githubusercontent.com/openai/openai-openapi/master/openapi.yaml`（3,562,936 字节，HTTP 200，2026-09-13 取回）；chat completions 的 `temperature` 属性逐字含 `minimum: 0`、`maximum: 2`、**`default: 1`**、`example: 1`，描述「What sampling temperature to use, between 0 and 2. Higher values like 0.8 will make the output more random, while lower values like 0.2 will make it more focused and deterministic.」——**区间 0–2 / 默认 1 / 「低值更确定」三项全部有规格原文**，原「保留待人工确认」可撤销。注：该页 `platform.openai.com/docs/api-reference/chat/create` 直连与代理**均 403**，故以规格文件为权威出处。〕
...
| 残余复核 | 上表「加厚」行遗留的悬置项：platform / help.openai.com 的 403 与「须人工复核后统一为当期规范地址」 | **已解决**：原地址**未失效，是 301 永久跳转**——`platform.openai.com/docs/guides/prompt-engineering` → `HTTP/1.1 301` → `location: https://developers.openai.com/api/docs/guides/prompt-engineering`；落地页 200（682,830 字节），`rel=canonical` 即新地址、`<title>Prompt engineering \| OpenAI API</title>`。主链接已改为新地址，原地址条内保留备查。旁证：openai-python 规格生成的类型定义中指南类引用**全部**为 `developers.openai.com/api/docs/…`（复取 20 条） |
| 残余复核 | 参考资料 6：「100 Token ≈ 75 英文单词」与「中文压缩比无官方数据」 | **已解决（改用归档快照）**。该页直连与代理均 403，取 `web.archive.org` **2026-09-01** 快照原文：「1 token is approximately 4 characters. 1 token is approximately three-quarters of a word. **100 tokens are approximately 75 words.** These are estimates, not exact counts.」→ 数字确认成立；同段「other languages can have different relationships…」反证中文压缩比确无官方数字。依据：`web.archive.org/web/20260901082009/https://help.openai.com/en/articles/4936856-…`（HTTP 200，取回 2026-09-13） |
| 残余复核 | 参考资料 7：`temperature` 默认值 1 未在规格 `default` 字段复核到 | **已解决**：官方 OpenAPI 规格 `openai/openai-openapi` 的 `openapi.yaml`（3,562,936 字节，HTTP 200）中 chat completions `temperature` 写明 `minimum: 0` / `maximum: 2` / **`default: 1`** / `example: 1`，描述含「Higher values like 0.8 … more random, while lower values like 0.2 … more focused and deterministic.」→ 区间、默认值、「低值更确定」三项均有规格原文。该页 `platform.openai.com/docs/api-reference/chat/create` 直连与代理**均 403**，故以规格文件为权威出处 |
| 残余复核 | 「官方域 403 路径无关、自动探测无判别力、只能人工浏览器复核」这一判断是否成立 | **已撤回（本次复核中途所得的错误结论，非上一轮原文）**：该判断**只在直连出口下成立**（直连时 platform = Cloudflare 挑战页 5483B、developers = Vercel `X-Vercel-Mitigated: deny` 59B，随机路径同响应）；**换本机 SOCKS 代理出口后判别力恢复**，真实路径 200、随机路径 404。上一轮的「须人工用浏览器复核」非事实错误，但**把可自动化的核验误判为必须人工**。依据：本机 curl 直连 vs 代理对照探测（2026-09-13） |
