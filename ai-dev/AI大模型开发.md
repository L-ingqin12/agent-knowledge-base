...
> [!warning] 更正（2026-09-13）：上表频率方向标反，且 dim3 / dim7 两行数值与公式不符（原表述为：`PE_dim0 … ← 低频, 变化慢`、`PE_dim7 … ← 高频, 变化快`）
> 按本文下方源码节的公式 `PE(pos,2i)=sin(pos/10000^(2i/d_model))`，**维度序号 i 越小 → 分母越小 → 频率越高**：前 8 维里 dim0/dim1 变化最快（分母 = 10000^(0/512) = 1.0，周期 2π ≈ 6.28），dim7 最慢（分母 ≈ 1.114）；推到末尾 dim510/511 才是最低频（分母 ≈ 9772）。故正确标签是 dim0/dim1 为「最高频, 变化最快」、维度序号越大频率越低。
> 上表中 dim0 行 [0.00, 0.84, 0.91, 0.14, -0.76] 与 dim1 行 [1.00, 0.54, -0.42, -0.99, -0.65] 与公式一致；dim3 行与 dim7 行不是 cos(pos/分母)（dim3 在 pos=0 必为 cos(0)=1.00）。正确值：
> - `PE_dim3` = [1.00, 0.57, -0.35, -0.97, -0.75]
> - `PE_dim7` = [1.00, 0.62, -0.22, -0.90, -0.90]
...
i=1 (维度2): PE(3,2) = sin(3 / 10000^(2/512)) = sin(3/1.04) =  0.260   ← 分母被取整成 1.04，正确分母 1.03663，正确值 0.2451
i=1 (维度3): PE(3,3) = cos(3 / 10000^(2/512)) = cos(3/1.04) = -0.966   ← 正确值 -0.9695
i=2 (维度4): PE(3,4) = sin(3 / 10000^(4/512)) = sin(3/1.08) =  0.370   ← 正确分母 1.07461，正确值 0.3428
i=2 (维度5): PE(3,5) = cos(3 / 10000^(4/512)) = cos(3/1.08) = -0.929   ← 正确值 -0.9394
i=3 (维度6): PE(3,6) = sin(3 / 10000^(6/512)) = sin(3/1.12) =  0.468   ← 正确分母 1.11397，正确值 0.4336
i=3 (维度7): PE(3,7) = cos(3 / 10000^(6/512)) = cos(3/1.12) = -0.884   ← 正确值 -0.9011
```

> [!warning] 更正（2026-09-13）：本块 6 个数值系误算（原表述为 dim2=0.260、dim3=-0.966、dim4=0.370、dim5=-0.929、dim6=0.468、dim7=-0.884，分母写作 1.04 / 1.08 / 1.12）
> 按本文件源码节的公式与 d_model=512，分母应为 10000^(2/512)=1.03663、10000^(4/512)=1.07461、10000^(6/512)=1.11397；且用文档自报的分母也复算不出上列数值（sin(3/1.04)=0.2542、cos(3/1.04)=-0.9670、sin(3/1.08)=0.3558、cos(3/1.08)=-0.9346、sin(3/1.12)=0.4467、cos(3/1.12)=-0.8947）。
> 正确手算结果（d_model=512, pos=3）：dim0=+0.1411、dim1=-0.9900、dim2=+0.2451、dim3=-0.9695、dim4=+0.3428、dim5=-0.9394、dim6=+0.4336、dim7=-0.9011（dim0 / dim1 两行原文正确，分母依次 1.00000 / 1.00000 / 1.03663 / 1.03663 / 1.07461 / 1.07461 / 1.11397 / 1.11397）。
> 证据：本机按文档自述公式独立复算（Python，2026-09-13）。
...
> [!warning] 更正（2026-09-13）：上一句两处不准确（原表述为「RoPE 结合了两者优点：可学习 + 相对位置 + 可外推更长序列」）
> ① **RoPE 不含任何可学习的位置参数**——它是用固定频率基（10000）构造旋转矩阵、直接作用在 Q/K 上的变换；"可学习"的只是 $W_q/W_k$ 投影，与选哪种位置编码无关。
> ② **朴素 RoPE 不能直接外推到训练长度之外**，"可外推"必须配合位置插值（Position Interpolation）/ NTK 缩放 / YaRN 等 `rope_scaling` 方案，不缩放直接用会显著劣化——原文缺这个边界条件。
> 准确表述：RoPE = 无参数的固定旋转变换，天然具有相对位置性质（QK 内积只依赖相对距离）；能否在训练长度之外保持性能取决于外推方案。
> 来源：[Rotary Embeddings: A Relative Revolution — EleutherAI Blog](https://blog.eleuther.ai/rotary-embeddings/)（2026-09-13 抓取，HTTP 200）；外推方案对照见 [[LLM架构进阶-从注意力变体到推理引擎]] 的「长上下文：位置编码外推」小节。
...
> [!warning] 更正（2026-09-13）：上块两个步数口径互斥，「主路 + 高速路」相加不能跨层相乘（原表述为「经过约 200 次乘法 → 0.9^200 ≈ 7×10^-10」与同段「第 1 层 dp = 0.9^95 = 0.000045」并存，以及「第95层: 主路 0.9 + 高速路 1.0 = 1.9」）
> - **口径统一**：按"每层 2 个子层（Attention + FFN）"记，GPT-3 的 96 层共 192 步，故"约 200 次乘法"与 95 步是两套算法，不能同时成立。无残差时第 1 层梯度 = 0.9^192 ≈ 1.6×10^-9（若按"每层 1 步"的口径则是 0.9^96 ≈ 4.0×10^-5，也必须全篇统一）。
> - **残差路径不能相加后跨层相乘**：`0.9 + 1.0 = 1.9` 是单层内两条路径之和；若把它当逐层因子，46 层得 1.9^46 ≈ 6.6×10^12（梯度爆炸），与同段"第 50 层 = 1.008"自相矛盾。
> - **正确机制**：有残差时梯度沿 $\prod (1 + f')$ 传播，恒等路径的贡献恒为 1.0（不随层数衰减）——这才是"残差保梯度"的来源；主路衰减项与恒等项在**同一层内相加**，跨层时应逐层相乘 $(1+f'_L)$，而不是把 $1.9$ 连乘。
> 证据：本机复算 0.9^95=4.498e-05、0.9^96=4.048e-05、0.9^192=1.639e-09、0.9^200=7.055e-10、1.9^46=6.648e+12（Python，2026-09-13）。
...
> 左侧无残差：梯度指数衰减（0.9^95 ≈ 0；统一到"每层 2 个子层"口径即 0.9^192 ≈ 1.6×10^-9，见上文更正块）。右侧有残差：高速路直传梯度（≈1.0），前面层也能学到。
...
**补疏漏（2026-09-13）— 归一化的现代演进：RMSNorm**

$$RMSNorm(x) = \frac{x}{\sqrt{\mathrm{mean}(x^2) + \epsilon}} \cdot \gamma$$

- 与 LayerNorm 的差别：**不减均值、没有 β 偏置**，只按均方根（RMS）缩放，再乘一组可学习缩放 $\gamma$。
- 取舍：省掉一次均值归约与一组 β 参数，kernel 更简单、吞吐更好；多数任务上精度与 LayerNorm 相当——所以 LLaMA / Qwen / Gemma 等现代 LLM 用它取代 LayerNorm，是本文件第 382 行"现代 LLM 差异点"清单里与 Pre-Norm、SwiGLU、RoPE 并列的一项。
- 归一化维度仍是"同一样本、不同特征"，因此同样不受 batch size 影响（对比上表）。
- 参考实现：PyTorch 已内置 [`torch.nn.RMSNorm`](https://docs.pytorch.org/docs/2.14/generated/torch.nn.RMSNorm.html)（2026-09-13 抓取，HTTP 200）。
...
- 现代 LLM 的差异点集中在：Pre-Norm/Post-Norm、激活函数 (ReLU→SwiGLU)、归一化 (LayerNorm→RMSNorm，见上文 [[#Layer Normalization — 深度理解]])、位置编码 (绝对→RoPE/ALiBi)，对比见 [[#不同模型对应不同的结构 训练数据 训练目标 训练时间]]
...
T=0.5 (低温, 更确定):  [0.92, 0.06, 0.02]  → 几乎只选第一个   ← 正确值 [0.976, 0.018, 0.007]
T=1.0 (标准):          [0.71, 0.20, 0.09]  → 按比例选         ← 正确值 [0.821, 0.111, 0.067]
T=2.0 (高温, 更随机):  [0.48, 0.29, 0.23]  → 相对均匀         ← 正确值 [0.605, 0.222, 0.173]
```

> [!warning] 更正（2026-09-13）：三档概率均与上一行的 $softmax(x_i/T)$ 定义不符（原表述为 T=0.5→[0.92,0.06,0.02]、T=1.0→[0.71,0.20,0.09]、T=2.0→[0.48,0.29,0.23]）
> 按 logits [3.0, 1.0, 0.5] 独立复算（本机 Python，2026-09-13）：T=0.5 → [0.9756, 0.0179, 0.0066]；T=1.0 → [0.8214, 0.1112, 0.0674]；T=2.0 → [0.6045, 0.2224, 0.1732]。定性结论「T 越高分布越平」正确，但 T=1.0 时首项差了 0.111（0.71 vs 0.82），教学示例应改用复算值。
...
class SwiGLU(nn.Module):
    ...
    和原版 ReLU FFN 的区别:
      ...
      - [更正 2026-09-13] 值域写错: 被压到 0~1 的是 σ(x) 而非 SiLU; SiLU(x)=x·σ(x) 平滑非单调, 值域 [-0.2785, +∞), 正区间近似线性
      - 比 ReLU 更平滑, 梯度流动更好
      - 缺点: 参数量多 50% (需要 3 个权重矩阵而非 2 个) —— 前提是中间维保持 d_ff=4d; 缩到 8d/3 则总量与 ReLU-4d 持平 (见下方更正块)
    """
    ...
    def forward(self, x):
        # SiLU(x) = x * sigmoid(x)
        gate = F.silu(self.W2(x))            # 门控信号: 平滑非单调, 值域 [-0.28, +∞)（原注 0~1 有误: 压到 0~1 的是 σ(x)）
        hidden = self.W1(x)                  # 主路径
        return self.W3(hidden * gate)         # 门控 × 主路径
```

> [!warning] 更正（2026-09-13）：SiLU 值域写错，且「参数多 50%」缺前提（原表述为「SwiGLU 有"门控"机制 (SiLU 输出在 0~1 之间)」与「缺点: 参数量多 50% (需要 3 个权重矩阵而非 2 个)」）
> - **值域**：`SiLU(x) = x·σ(x)`，σ∈(0,1) 但 x 无界，所以 SiLU 平滑、**非单调**，值域为 **[-0.2785, +∞)**（最小值出现在 x≈-1.278，正区间近似线性）。被压到 0~1 的是 σ(x)，不是 SiLU。SwiGLU 的门控作用来自 `SiLU(xW_gate)` 与 `xW_up` 的**逐元素相乘**，不是把激活压到 0~1。
>   来源：[torch.nn.SiLU — PyTorch 官方文档](https://docs.pytorch.org/docs/2.14/generated/torch.nn.SiLU.html)（2026-09-13 抓取，HTTP 200）；SiLU 最小值 -0.2784645 由本机复算（Python，2026-09-13）。
> - **参数量**：「多 50%」只在中间维保持 $d_{ff}=4d$ 时成立：ReLU FFN = $2d \cdot 4d = 8d^2$；SwiGLU($d_{ff}=4d$) = $3d \cdot 4d = 12d^2$（+50%）。主流实现把 SwiGLU 的中间维缩到 $(2/3)\cdot 4d = 8d/3$，总量回到 $8d^2$，与 ReLU-4d 版持平，代价是中间维变窄——LLaMA / PaLM 一系即如此。
>   来源：[torchtitan `compute_ffn_hidden_dim`（PyTorch 官方仓库）](https://raw.githubusercontent.com/pytorch/torchtitan/main/torchtitan/models/common/feed_forward.py)（Llama3/4 系取 `hidden_dim = int(2*4*dim/3)`，docstring 明写 "applies the 2/3 scaling"）、[SwiGLU — zubnet.ai wiki](https://zubnet.ai/wiki/en/SwiGLU/)（"To keep parameter count constant, the intermediate dimension is typically reduced from 4×model_dim to (8/3)×model_dim"）。均为 2026-09-13 抓取，HTTP 200。
...
| 类型 | 原问题 | 处置与依据 |
|---|---|---|
| 纠错 | 「节奏指纹」表把 dim0 标为「低频, 变化慢」、dim7 标为「高频, 变化快」，且 dim3 / dim7 两行数值不是 cos(pos/分母) | 表后就地加更正块：按本文公式**维度序号越大频率越低**，dim0/dim1 才是最高频（分母 1.0），dim7 最慢（分母 ≈1.114）；补正确行 dim3=[1.00,0.57,-0.35,-0.97,-0.75]、dim7=[1.00,0.62,-0.22,-0.90,-0.90]（本机按文档自述公式复算） |
| 纠错 | 手算 pos=3 前 8 维 PE：分母取整成 1.04/1.08/1.12，dim2=0.260、dim4=0.370、dim6=0.468、dim7=-0.884 均为错值 | 每个错值后就地标注正确值：分母应为 1.03663/1.07461/1.11397，正确序列 0.1411 / -0.9900 / 0.2451 / -0.9695 / 0.3428 / -0.9394 / 0.4336 / -0.9011（Python 独立复算） |
| 纠错 | softmax + 温度三档概率（0.92/0.71/0.48）与同节 $softmax(x_i/T)$ 定义不符 | 三行就地标注复算值 [0.976,0.018,0.007] / [0.821,0.111,0.067] / [0.605,0.222,0.173]，并加更正块说明定性结论不变 |
| 纠错 | 「RoPE 结合了两者优点：可学习 + 相对位置 + 可外推」两处不准确 | 加更正块：RoPE 无任何可学习位置参数；朴素 RoPE 不能直接外推，须 rope_scaling（PI/NTK/YaRN）。来源 [EleutherAI Rotary Embeddings](https://blog.eleuther.ai/rotary-embeddings/) |
| 纠错 | 残差梯度实例同时使用 95 步与 200 步两套口径，且把「主路 0.9 + 高速路 1.0 = 1.9」当逐层因子 | 加更正块统一口径：96 层 × 2 子层 = 192 步，无残差第 1 层 = 0.9^192 ≈ 1.6e-9；有残差时恒等路径贡献恒为 1.0，跨层按 ∏(1+f′) 相乘（本机复算 0.9^192、1.9^46 等佐证矛盾） |
| 纠错 | SwiGLU 段称「SiLU 输出在 0~1 之间」，代码注释写「门控信号: 0~1」 | docstring 与注释就地更正：SiLU 平滑非单调，值域 [-0.2785, +∞)，被压到 0~1 的是 σ(x)。来源 [torch.nn.SiLU — PyTorch 文档](https://docs.pytorch.org/docs/2.14/generated/torch.nn.SiLU.html) |
| 补疏漏 | 「参数量多 50%」缺前提，读者会以为用 SwiGLU 必然多 50% 参数 | 补 `d_ff=4d` 前提与 `8d/3` 取舍（12d² → 8d²，与 ReLU-4d 持平）。来源 [torchtitan feed_forward.py](https://raw.githubusercontent.com/pytorch/torchtitan/main/torchtitan/models/common/feed_forward.py)、[zubnet SwiGLU wiki](https://zubnet.ai/wiki/en/SwiGLU/) |
| 补疏漏 | 全文从未出现 RMSNorm，而它是「现代 LLM 差异点」清单里最常被追问的一项 | LayerNorm 节补 RMSNorm 公式与取舍，并在第 382 行差异点清单并列。来源 [torch.nn.RMSNorm — PyTorch 文档](https://docs.pytorch.org/docs/2.14/generated/torch.nn.RMSNorm.html) |

回链：[[CORRECTIONS]]（本库更正总表）· 相关：[[LLM架构进阶-从注意力变体到推理引擎]]、[[AI-Dev-KB-Home]]
