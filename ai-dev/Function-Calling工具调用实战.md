...
> [!success] 残余复核（2026-09-13）：参考资料 1 的悬置项（「该地址当前是否仍是函数调用指南的规范地址」）**已结案**——**原地址并未失效，而是 301 永久跳转到新主域**；当期规范地址为 `https://developers.openai.com/api/docs/guides/function-calling`，条内主链接已按此更新，原地址保留备查。
> **取回证据（2026-09-13 实测）**：
> - `https://platform.openai.com/docs/guides/function-calling` → **HTTP/1.1 301 Moved Permanently**，`location: https://developers.openai.com/api/docs/guides/function-calling`；
> - 落地页 **HTTP 200**（1,423,346 字节），`<link rel="canonical" href="https://developers.openai.com/api/docs/guides/function-calling">`、`<title>Function calling | OpenAI API</title>`；
> - 同主域随机路径对照 → **404**（证明 200/404 判别有效，不是整站兜底页）。
> **第二条独立证据（规格侧）**：OpenAI 官方 OpenAPI 规格生成的 SDK 类型定义里，指南类引用**全部**落在 `developers.openai.com/api/docs/…`（复取 20 条），`platform.openai.com` 只剩 `tokenizer` 一条工具页——属**主域整体迁移**，非单点推测。依据 `api.github.com/repos/openai/openai-python/contents/…`（`function_definition.py` blob `8837687028ce`、`completion_create_params.py` blob `49170b76e262`，仓库 HEAD `e12b81d3bbf6`）。
> **⚠️ 本轮自纠（撤回一条本次复核中途得出的错误结论）**：本次复核的第一版曾在本文写下「两域对本环境均为**路径无关的整站 403**，403 对路径零信息量，自动探测无判别力，只能人工用浏览器复核」——**该结论不成立，此处撤回**。它**只在直连出口下**成立（不经代理时 platform 回 Cloudflare 挑战页 5483B、developers 回 Vercel `X-Vercel-Mitigated: deny` 59B，且随机路径同响应）；**换出口（本机 SOCKS 代理）后判别力完全恢复**：真实路径 200、随机路径 404。同理，上一轮把本条记为「须人工用浏览器复核」并非事实错误，但**把本可自动完成的核验误判成必须人工**——手段选择被单一出口的经验锁死了。**教训**：「直连被边缘节点拒绝」**不等于**「无法核验」，先换出口复测再下结论；该错误的形状与 [[CORRECTIONS]] C-012「拿本机绿测当跨平台验收」同族（**把单一环境的观察当成对象自身的属性**），只是这次锁死判断的维度是网络出口而非操作系统。

1. [OpenAI Function calling 官方指南](https://developers.openai.com/api/docs/guides/function-calling) — "模型调用 → 执行工具 → 回传结果"流程与并行函数调用的官方说明。〔**2026-09-13 残余复核：已解决——原地址是 301 跳转，主链接已改为当期规范地址**。实测（2026-09-13，经本机 SOCKS 代理取回）：`https://platform.openai.com/docs/guides/function-calling` → **HTTP/1.1 301 Moved Permanently**，`location: https://developers.openai.com/api/docs/guides/function-calling`；落地页 **HTTP 200**（1.42 MB），`<link rel="canonical" href="https://developers.openai.com/api/docs/guides/function-calling">`，`<title>Function calling | OpenAI API</title>`。**原地址为 `https://platform.openai.com/docs/guides/function-calling`（现 301 永久跳转，仍可用）**，保留备查。〕
...
6. [OpenAI API Reference: chat/completions](https://platform.openai.com/docs/api-reference/chat/create) — `parallel_tool_calls` 与 `temperature`（默认 1，低值更确定性）的官方参数定义；〔**2026-09-13 残余复核：部分定论**——该路径直连与经代理**均 403**，故**无法观察它是否发生跳转**。**该 403 不携带路径信息**：同族对照实测 `platform.openai.com/docs/api-reference/zzz-bogus-not-a-page`（随机不存在路径）**同为 403**（响应体 9891 字节，与本条 9843 字节同量级），而同主域 `/docs/api-reference` 索引页为 **200**（348,776 字节）——即 **api-reference 的**明细路径族**被整族拦截，单点 403 既不能证明该页存在、也不能证明其失效**。可确证的只有迁移方向：新主域同源路径 `https://developers.openai.com/api/reference/resources/chat/subresources/completions` 实测 **HTTP 200**，且规格生成的 SDK 类型定义引用的正是 `developers.openai.com/api/reference/resources/chat/subresources/completions/streaming-events`。故本条地址**应随主域迁移更新**，但**改写前仍须一次真实浏览器核对**（旧路径的 301 目标本次不可观测）。参数本身的权威定义见第 2 条 `openapi.yaml`；其中 `temperature` 的 `default: 1` 已在 [[Prompt-Engineering入门与Demo]] 第 7 条本轮复核中核到原文。〕
7. [OpenAI Help Center: Function calling in the Chat Playground](https://help.openai.com/en/articles/9492280-function-calling-in-the-chat-playground) — 函数调用双轮交互过程的可视化演示。〔**2026-09-13 残余复核：已解决（归档取证）**——help.openai.com 对本环境 403，改取归档快照 `https://web.archive.org/web/20260701195355/https://help.openai.com/en/articles/9492280-function-calling-in-the-chat-playground`（快照日 **2026-07-01**，取回 2026-09-13，HTTP 200，44,292 字节）：页面标题《Function calling in the Chat Playground》，内容为**配置操作指南**——「To add a new function, click "+Tools > Functions" in the configuration panel … for Responses API or, simply, "+Function" for Chat Completions API」，另有 Beta「Generate」由自然语言生成 JSON schema。**原注「双轮交互过程的可视化演示」与内容不符，应为「在 Playground 中配置并调用函数的操作指南」**。〕
...
| 残余复核 | 上表「加厚」行遗留的悬置项：参考资料 1 的 `platform.openai.com/docs/guides/function-calling` 是否仍是规范地址（原文「须人工复核后改当期地址」） | **已解决**：该地址**未失效，是 301 永久跳转**——实测 2026-09-13：`platform.openai.com/docs/guides/function-calling` → `HTTP/1.1 301` → `location: https://developers.openai.com/api/docs/guides/function-calling`；落地页 200（1.42 MB），`rel=canonical` 即该新地址、`<title>Function calling \| OpenAI API</title>`。主链接已改为新地址，原地址条内保留备查。旁证：openai-python 规格生成的类型定义中指南类引用**全部**为 `developers.openai.com/api/docs/…`（复取 20 条），`platform.openai.com` 仅剩 `tokenizer`；blob `8837687028ce` / `49170b76e262`，HEAD `e12b81d3bbf6` |
| 残余复核 | 「403 路径无关、自动探测无判别力，只能人工浏览器复核」这一判断是否成立 | **已撤回（本次复核中途所得的错误结论，非上一轮原文）**：该判断**只在直连出口下成立**（直连时 platform = Cloudflare 挑战页 5483B、developers = Vercel `X-Vercel-Mitigated: deny` 59B，随机路径同响应）；**换本机 SOCKS 代理出口后判别力恢复**，真实路径 200、随机路径 404。上一轮的「须人工用浏览器复核」非事实错误，但**把可自动化的核验误判为必须人工**。处置：条内与本节均已改写为「换出口取回 + 200/404 对照」的证据口径。依据：本机 curl 直连 vs 代理对照探测（2026-09-13） |
| 残余复核 | 参考资料 6（`platform.openai.com/docs/api-reference/chat/create`）的 403 悬置 | **部分已解决**：旧路径直连与代理**均 403**，且**该 403 不携带路径信息**（随机不存在路径 `…/api-reference/zzz-bogus-not-a-page` 同为 403/9891B，而同主域 `/docs/api-reference` 索引 200/348,776B）→ **该明细路径族被整族拦截，单点 403 两头都证不了**，故不写成结论。可确证的只有迁移方向：新主域 `developers.openai.com/api/reference/resources/chat/subresources/completions` 实测 200，规格生成的 SDK 类型定义引用的正是该路径族。**结论**：地址应随主域迁移更新，改写前仍须一次真实浏览器核对。依据：本机 curl（代理，含随机路径对照）+ openai-python 规格（2026-09-13） |
| 残余复核 | 参考资料 7（help.openai.com 9492280）的 403 悬置，及原注「双轮交互过程的可视化演示」是否属实 | **已解决（归档取证）**：取 `web.archive.org` **2026-07-01** 快照（HTTP 200，44,292 字节），页面内容实为**配置操作指南**（`"+Tools > Functions"` / `"+Function"` / Beta「Generate」生成 JSON schema），**与原注不符，已在条内改写**。依据：`web.archive.org/web/20260701195355/https://help.openai.com/en/articles/9492280-…`（取回 2026-09-13） |
