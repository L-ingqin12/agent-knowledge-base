...
> [!success] 残余复核（2026-09-13）：工作单把本页 L164-166、L211 判为「未验证小节下的未决项」，**实为误报，本页无未决项**。
> - 这四行全部落在**示例输出代码块内**：L164-166 是 `security-auditor` 返回的**审计报告样例**（`位置: src/auth.js:33` / `修复: …` / `...` 三个占位行），L211 是紧随代码块之后的 `---` 分隔线。
> - 误判机制可复原：L163 的小节标题是「[W-01] resetPassword 函数**未验证**旧密码」，簇扫描把其中的「未验证」当成「未验证小节」的表头，于是把其后的代码块行收进清单。这是**关键词子串误命中**，不是文档里有待办。
> - 判据：本页通篇是**状态机回环的演示脚本**（用户请求 → 派发 → 门控 → 重试 → 通过），`src/auth.js` 是虚构示例文件，不存在于本库；正文并无「待确认 / 须人工复核」类主张。
...
| 类型 | 原问题 | 处置与依据 |
|------|--------|-----------|
| 排除 | L164-166 / L211 被判为「未验证小节下的待确认项」 | 四行均在示例代码块内（审计报告样例 + 分隔线）；L163 标题里的「未验证旧密码」被扫描当作小节名，属子串误命中；本页为演示脚本，无未决项 |
