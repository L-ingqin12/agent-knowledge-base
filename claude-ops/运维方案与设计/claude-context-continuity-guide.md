...
> [!warning] 口径补充（2026-09-13）：上面的 token 阈值（≤200 / ≤500 / < 3%）是**设计目标**，原文未给量测方式。要保留这些阈值，需写明计数来源与口径：单次写入用 `/context` 面板的 token 数，或后端返回的 usage 字段，或用本地 tokenizer 对写入片段计数，并全程固定同一口径；「任务总 tokens 的 3%」的分母定义为**同一任务从开始到结束的累计输入 + 输出 tokens（含恢复回合）**，不是单回合 tokens。§6.1 的 `~3000 / ~2000 tok` 同属估算，未附样本量与采集命令。
...
## 十一、语境连续性验收（补 2026-09-13）

原文 §5 / §9 给了恢复 Prompt 模板与 `resume-prompt-header.txt` 的可执行步骤，但没有「恢复后语境真的接上了」的判据。按下表做一次**可复现中断**即可验收（此表为设计判据，尚未记录真机样本）。

| # | 验收用例 | 操作 | 通过判据 |
|---|----------|------|----------|
| 1 | 恢复顺序正确 | 在「完成分析 → 进入修改」后 kill 会话（或断网），再用 §4.3 / §九 的恢复 Prompt 拉起 | 恢复后的**第一个工具动作**是读 `context-dump.md`，随后读 `task-state.json`（即 §§4.1 / 7.1 规定的顺序） |
| 2 | 思路不跑偏 | 同一场景中观察是否重新做技术选型 | 不重新选择 Decisions 表中被否定的方案；若确需改决策，必须先给出「新信息明确证明原决策错误」的理由 |
| 3 | 恢复开销量级 | 记录恢复首个回合的输入 token 数 | 落在 `~300 tok` 量级（只读 1 个文件）；若接近 `~3000 / ~5000 tok`，说明在重读文件、重做分析，外部大脑实际未被加载 |
| 4 | 写入开销受控 | 按 §3.3 的口径统计单次与累计写入 | 单次 ≤200 tok、累计 ≤500 tok、总开销 < 任务总 tokens 的 3%（计数口径见 §3.3 补充块） |

> 判据来源：本文 §§3.3 / 4.1 / 6.1 / 7.1 的自述指标，以及 `AGENTS.md` 部署四规则（部署前验证 / 日志可审计）。
...
| 类型 | 原问题 | 处置与依据 |
|---|---|---|
| 补疏漏 | §5 与 §9 给出恢复 Prompt 模板与 `resume-prompt-header.txt` 的可执行步骤，但全文无恢复效果的验收判据 | 新增 §十一 验收表（首个动作读 `context-dump.md` → `task-state.json`、不重选被否定方案、恢复开销 ~300 tok 量级、写入开销受控），并在 §九 末加指针 |
| 加厚 | §3.3 的 token 阈值（≤200 / ≤500 / < 3%）与 §6.1 的估算没有量测方式与分母定义 | §3.3 加「口径补充」块：计数来源（`/context` 面板、后端 usage 或本地 tokenizer）、固定同一口径、「3%」的分母定义，并标明这些数字是设计目标、未附样本 |
