...
## 六·五、错误记忆协议（2026-09-12 新增）

> [!danger] ⛔ 出结论前先回查 [[CORRECTIONS]]
> 本库维护一份**可检索的方法论错误记忆库** [[CORRECTIONS]]。在把任何**判断性结论**写入交付物之前，先回查其速查索引一次；
> **命中即按对应规则重检**。

**为什么单列一节**：本库已有 [[MEMORY-INDEX]]（架构模式）与散落的「教训」小节，但**没有一处记录「判断错过的结论」**。错误散落在正文里 = 不可检索 = 必然重犯。
该库截至 2026-09-12 的 14 条记录**同属一个根源**：**把「观察到的现象」直接当成「结论」**。

**三条硬性要求**

| # | 要求 | 反例 |
|---|------|------|
| 1 | **判定漏洞必须成对读取**「受影响范围」与「修复版本」两个值 | 只读到 `Affected: >=8.0.0` 就断言「无补丁」——漏读下一行 `Patched: >=8.4.0` |
| 2 | **判定暴露面必须落到**「认证方式 / 凭据强度 / 是否复用」三个具体问题 | 见端口开放就报严重——未验证认证方式即定级 |
| 3 | **下「未发现」结论前先证明扫描器看得见目标** | `-Recurse` 在 junction 农场返回 0，被误读为「干净」 |

**录入义务**：凡**已输出给用户或写入交付物**的错误结论、或**可复现的推理模式错误**，必须录入 [[CORRECTIONS]]（含「错在哪一步」与「检测方法」）。
**撤回的结论同样必须录入**——撤回比错误本身更有价值，它标出了哪类推理会产出假阳性。

> [!note] 与三级记忆模型的关系
> 本协议遵循 [[agent-memory-context-knowledge-design]] 的分层：
> **正文留在 L3（[[CORRECTIONS]] 文件，版本化、可检索）**，**只有触发规则进 L1（本节的十几行）**。
> 该文档明确警告「把长期知识硬编码进系统提示词是最高频反模式」，故此处只放指针与硬性要求，不复制条目内容。
