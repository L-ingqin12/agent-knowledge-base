...
| 子目录 | 原仓库 | 内容 |
|--------|--------|------|
| `lsp-learn/` | `L-ingqin12/lsp-learn` | LSP（Language Server Protocol）学习指南 5 篇：概述与架构 / 核心功能（文档同步·诊断·补全·跳转·Code Action）/ 用 Python+pygls 从零实现服务端 / 进阶（增量同步·进度通知·多工作区·性能）/ 生态全景，附 `demo/`（client 含语法高亮、server） |

> 与工具链主干的关系：LSP 是编辑器与语言服务的通用协议层，归入**语言与工具链**，与 [[LLVM编译器基础设施]]、[[LibC与动态链接]] 同域。
