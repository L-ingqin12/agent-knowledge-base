...
> [!success] 残余复核（2026-09-13）：②③④ 本轮就地定论；① 仍开放（判据见下）。
> - **③ 已定论**：可增长栈这条路线上，主流实现里真正大规模用的是 Go，而它走的是**拷贝栈（连续栈）**而非分段栈——本机 Go 源码 `src/runtime/stack.go` 里栈增长函数是 `func copystack(gp *g, newsize uintptr)`、goroutine 状态有 `_Gcopystack`，全 runtime 检索不到 "segmented stack" 痕迹（分段栈是 Go 1.3 之前的历史方案）。C++ 生态的有栈协程（ucontext／Boost.Context 系）一律**固定栈不做增长**，这正是本文 §五"默认栈小、深递归爆栈"陷阱的成因。判据（复跑）：`grep -n copystack $(go env GOROOT)/src/runtime/stack.go`。
> - **④ 已定论**：6.x 内核**没有**给用户新增 epoll 告警/事件类型，原问法里的"EPOLLET 边沿+优先级"本身是把概念并在一起了（`EPOLLET` 是触发模式、`EPOLLPRI` 是带外数据事件，都是 2.6 时代就有的东西）。依据：本机内核头 `linux-libc-dev` 6.1.162 的 `<linux/eventpoll.h>` 中事件位只有 `EPOLLIN/PRI/OUT/ERR/HUP/NVAL/RDNORM/RDBAND/WRNORM/WRBAND/MSG/RDHUP`，新增的 `EPOLL_URING_WAKE`(1<<27) 在头文件里注释明确写着 "Internal flag - wakeup generated by io_uring, used to detect recursion back into the io_uring poll handler."，不是给用户的事件；man-pages 6.03 的 `epoll_ctl(2)` 全文 "(since Linux …)" 标注也只到 2.6.2 / 2.6.17（`EPOLLRDHUP`）/ 3.5 / 4.5（`EPOLLEXCLUSIVE`）为止。判据（复跑）：`grep -n EPOLL /usr/include/linux/eventpoll.h` + `zcat /usr/share/man/man2/epoll_ctl.2.gz | grep -o "(since Linux [0-9.]*)"`。
> - **① 仍开放**：SQPOLL 的最优核数是**负载相关**的经验值，没有离线可抄的答案。判据：在 ≥8 核、双 NUMA 节点的机器上用 `fio`（`io_uring` engine，`hipri`/`sqthread_poll=1`）扫 `--sqthread_poll_cpu` 与 `--numjobs` 组合，记录 iops 与 CPU 占用，并先打印 io_uring features 位确认 SQPOLL 真的生效（老内核会静默降级，见 §三）。
> - **② 已定论（权威源已锚定到页级）**：DPDK 官方文档本身就是那份矩阵——本次取回 `doc.dpdk.org/guides/nics/`（2026-09-13）确认：virtio 有独立 PMD 页且带稳定小节（「Virtio Implementation in DPDK」「Features and Limitations of virtio PMD」「Prerequisites」「Virtio with qemu virtio Back End」「Virtio PMD Rx/Tx Callbacks」「Interrupt mode」）；云厂商虚拟网卡在 DPDK 里是**一等 PMD**，TOC 中直接可见 `ENA Poll Mode Driver`（AWS ENA）、`VDEV_NETVSC driver`（Hyper-V/Azure netvsc）以及 Paravirtual VMXNET3，另有 vDPA 设备驱动与 Features Overview 页作逐特性对照。故"矩阵更新"的正确做法是：升级 DPDK 时对该目录的 Features Overview 与各厂商 PMD 页做 diff（别查二手汇总），落地仍以 `dpdk-devbind.py --status` + `testpmd` 实测 pps 复核。
...
| 残余复核 | §七③ 有栈协程栈增长方案（分段栈 vs 拷贝栈）取舍现状 | 定论：主流可增长栈实现（Go）走**拷贝栈**——本机 Go 源码 `src/runtime/stack.go` 的 `copystack`/`_Gcopystack`，runtime 内无 segmented stack 残留；C++ 有栈协程（ucontext/Boost.Context）固定栈不增长 |
| 残余复核 | §七④ epoll 在 6.x 内核新增告警类型（EPOLLET 边沿+优先级）的演进 | 定论为**无此演进**：内核 6.1 UAPI 头里事件位清单止于 2.6 时代那批 + 4.5 的 `EPOLLEXCLUSIVE`，6.x 唯一新增 `EPOLL_URING_WAKE` 头文件注明是 internal flag；man-pages 6.03 的 since 标注只到 4.5。"边沿/优先级"分别属触发模式与 `EPOLLPRI`，非新事件类型 |
| 残余复核 | §七① io_uring 多核 SQPOLL 最优核数 / §七② DPDK virtio PMD 矩阵 | ① 仍开放（负载相关经验值，须 ≥8 核双 NUMA 机跑 `fio` + `--sqthread_poll_cpu` 扫描并先验 features 位）；② 已定论：权威源锚定到 `doc.dpdk.org/guides/nics/` 的 virtio PMD 页、Features Overview 与厂商 PMD（ENA/NETVSC/VMXNET3/vDPA），"更新"=升级时对该目录做 diff |
| 残余复核 | §三 io_uring “待确认内核 5.15+ vs 6.x” | 非待办：2026-09-13 补完时已就地收口（改指 [[操作系统八股]] §五 附的能力×最低内核版本表，并补 liburing 静默降级前置检查），本轮无残留 |
